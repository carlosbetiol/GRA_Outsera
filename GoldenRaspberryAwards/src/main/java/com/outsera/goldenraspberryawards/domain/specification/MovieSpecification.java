...
public class MovieSpecification {
    ...
    public static Specification<Movie> byCriteria(MovieCriteria criteria) {

        return (root, query, builder) -> {
            List<Predicate> predicates = new ArrayList<>();

            Join<Movie, Studio> studioJoin = root.join("studios", JoinType.INNER);
            Join<Movie, Producer> producerJoin = root.join("producers", JoinType.INNER);
            Join<Movie, MovieAward> movieAwardJoin = root.join("movieAwards", JoinType.LEFT);
            ...
            if (criteria.getAwardYear() != null && !criteria.getAwardYear().isEmpty()) {
                predicates.add(movieAwardJoin.get("awardYear").in(criteria.getAwardYear()));
            }
